...
## A few important notes 
  ...
  * If you copy the entire .rmd of this example file into your new R markdown file, remove the first part of this example .rmd file (the beginning of .rmd file starting and ending with `---` that record your name, date and title of the page. You should retain your own .rmd header as the title of your file).     
  ...
  * Also, a quick note for formatting - header specification to make prettier headers and sub headers   
      When specifying header, we use `#` (large header) or `#####` (small header) and somewhere between, depening on the number of `#`s. Space is needed between `#` and header description. Without the space, `#` is treated literal, rather than the special character controlling the size of header.  Example below:
    
#### Header with 4`#`s, with space in the .rmd markdown file, with correct header formatting    
####Header with 4`#`s, without space in the markdown file, interpreted literally as charateror "#", rather than the indicator of header 
...
You will notice the header line turns into blue fonts when interpreted as header in the markdown file.  Also, it is good to give a space after the header statement just to indicate the end of the header and to create a new line.  
  ...
  * A side note (not related to this assignment), you have some control in what to display in R codes within chunk` ```{r}`  
